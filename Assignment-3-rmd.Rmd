...
#Acoustic measures:
InformedEffectMean <- c(-1.26, -0.55, -0.75, 1.89, 0.25, 0.05, 0, 0, 0, 0) #taken from Parola et al (2020). 
SkepticEffectMean <- rep(0, 10) #noise condition, repetition of 0s ten times.

#Defining individual variability from population across trials and measurement error.
IndividualSD <- 1
TrialSD <- 0.5
Error <- 0.2
```
...
for (i in seq(10)) {
  temp_informed <- tibble(
    ID = seq(n),
    TrueEffect = rnorm(n, InformedEffectMean[i], IndividualSD), #for the participant; sampled from the mean and individual sd. Has atypicalities in voice.
    Variable = paste0("v", i)) #creating the variable
  temp_skeptic <- tibble( #controlled data set, where we have no effect, no underlying population effect
    ID = seq(n),
    TrueEffect = rnorm(n, SkepticEffectMean[i], IndividualSD), #no real atypicalities in voice.
    Variable = paste0("v", i))
  if (i == 1) { #if this is the first time you run, just take that as your data set, if not, bind it.
    d_informed_true <- temp_informed #there are atypicalities in voice
    d_skeptic_true <- temp_skeptic #there are no atypicalities in voice
  } else {
...
for (i in seq(nrow(d_informed))) { #loop each row, identify the measurement.
  d_informed$measurement[i] <- ifelse(d_informed$Group[i] == "Schizophrenia", 
                                      rnorm(1, rnorm(1, d_informed$TrueEffect[i]/2, TrialSD), Error), #effect for schizophrenia.
                                      rnorm(1, rnorm(1, (-d_informed$TrueEffect[i])/2, TrialSD), Error)) #-, as control is half the difference below zero.
...
plot_informed_vars #There are atypicalities in voice. For instance - v1 is the proportion of spoken time. v4 is duration of pauses.
plot_skeptic_vars #There is no voice atypicalities in thi data set, both distributions for each group are overlapping, and the differences we see are merely because of the noise/error in the data when it is simulated. Running the simulation again would change how distributions would look.
...
In terms of data budgeting, 80% of the data will be for training, and 20% for testing.
Need to identify 20 % of participant pairs and set them aside.
...
Scales the training data (one at a time, keeping as a note of how it works)
```{r}
#INFORMED

m_1 <- mean(train_informed$v1)
sd_1 <- sd(train_informed$v1)
train_informed <- train_informed %>%
  mutate(v1_s = (v1 - m_1) / sd_1)
test_informed <- test_informed %>% 
  mutate(v1_s = (v1 - m_1) / sd_1)
#---------------------------------------------------------------
#SKEPTIC

m_2 <- mean(train_skeptic$v1)
sd_2 <- sd(train_skeptic$v1)
train_skeptic <- train_skeptic %>%
  mutate(v1_s = (v1 - m_2) / sd_2)
...
Scales and centers all of the variables (TIDYMODELS)
```{r}
...
```{r}
#One predictor to start with:
p_range_f0 <- bf(Group ~ 1 + v2) 


p_range_f_1 <- bf(Group ~ 1 + v2 + v1 + v3 + v4 + v5 + v6 + v7 + v8 + v9 + v10) 
get_prior(p_range_f_1, train_informed_s, family = bernoulli) #given we have a certain pitch range, we expect a certain probability of someone having schizophrenia. Bernoulli - has 0 or 1 as outcomes, takes one trial at a time. 
p_range_p <- c(
  prior(normal(0, 1), class = Intercept),
  prior(normal(0, 0.3), class = b) #maybe should be more conservative
)
...
p1_10_m3 <- grid.arrange(informed_pp_b_v1_3, informed_pp_b_v2_3, informed_pp_b_v3_3, informed_pp_b_v4_3, informed_pp_b_v5_3, informed_pp_b_v6_3, informed_pp_b_v7_3, informed_pp_b_v8_3, informed_pp_b_v9_3, informed_pp_b_v10_3)
informed_pp_intercept_3
informed_pp_sd_3
...
All predictors: (Doesn't tell alot - as the amount of Sz and C number of people is the same.)
```{r}
pitch_m_skeptic <- brm(
  p_range_f_1, 
...
#Informed model varying intercept
...
```{r}
#Skeptic model with all predictors
...
for (i in seq(2000)) {
  train_skeptic_s$Predictions0 <- as.factor(ifelse(train0[i,]> 0.5, "Schizophrenia", "Control"))
  test_skeptic_s$Predictions0 <- as.factor(ifelse(test0[i,]> 0.5, "Schizophrenia", "Control"))
...
#Skeptic model varying intercept
...
How different choices when we fit our models affect the  ability to predict the test set?
```{r}
...
```
In this case, having looser priors for the specific model do not matter? Maybe there is a marginal difference between 0.2 and 0.3, and it might be better to have a prior of 0.3, but it does not show a significant difference.
...
#Building a model of varying slopes.
#colnames(e_train_s)
